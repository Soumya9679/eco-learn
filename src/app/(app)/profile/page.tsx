...
export default function ProfilePage() {
  ...
  const statItems = [
    ...
    { label: "Challenges", value: user.progress.challengesCompleted, icon: Target, color: "text-[var(--primary-400)]", bg: "rgba(52,211,153,0.1)" },
  ...
  const ecoStats = [
    ...
    { label: "Waste Diverted", value: `${user.stats.wasteDiverted}kg`, icon: Trash2, color: "text-[var(--primary-400)]" },
    { label: "Trees Planted", value: user.stats.treesPlanted, icon: TreePine, color: "text-[var(--primary-400)]" },
  ];

  return (
    <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-6">
      {/* Profile Hero */}
      <motion.div variants={staggerItem} className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[var(--primary-500)] via-[var(--primary-600)] to-blue-900 p-6 sm:p-8 text-white shadow-[0_0_20px_var(--shadow-glow)]">
              ...
              <div className="divide-y divide-white/5">
                {activities.map((act) => (
                  <div key={act.id} className="flex items-center gap-3 px-4 py-3">
                    <div
                      className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--primary-400)]"
